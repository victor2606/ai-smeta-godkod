# План реализации: SQLite + Claude Code Agent для поиска расценок
...
**Статус:** Обнаружены критические несоответствия между компонентами
**Дата обнаружения:** 2025-10-19
**Блокирует:** Задачу 2.3 (Популятор базы данных)

### Проблемы:

1. **DataAggregator не извлекает cost поля** → Задача 2.2.1
   - Schema требует: `total_cost`, `materials_cost`, `resources_cost`
   - DataAggregator: ❌ не создает эти поля
   - Excel содержит: ✅ `Общая стоимость по позиции`

2. **Schema не имеет колонки composition** → Задача 2.2.2
   - DataAggregator создает JSON поле `composition`
   - Schema: ❌ нет колонки для хранения

3. **Несовместимость resource_type** → Задачи 2.2.2 + 2.2.3
   - Schema CHECK constraint: `IN ('material', 'labor', 'machinery', 'equipment')`
   - DataAggregator: `row_type = 'Ресурс'` (не подходит!)

### План исправления:

```
Задача 2.2.1 (DataAggregator) ┐
                               ├→ Задача 2.3 (DatabasePopulator) → Задача 2.4
Задача 2.2.2 (Schema)          │
                               │
Задача 2.2.3 (Resource mapper) ┘
```

**Задачи 2.2.1, 2.2.2 могут выполняться параллельно**
...
**Блок 1 (после Задачи 2.4 - готовая БД):**
...
ФАЗА 1 ✅ → Задача 2.1 → Задача 2.2 → Задача 2.3 → Задача 2.4 → Задача 7.1
                                                       ↓
                                         Задачи 3.1, 3.2, 3.3 (параллельно)
                                                       ↓
...
**Зависимости:** ⏳ После завершения Задачи 2.1
**Параллельно с:** Нет (зависит от результатов загрузчика)
...
**Затронутые файлы:**
- `src/etl/data_aggregator.py` (новый)
- `src/utils/text_processor.py` (новый - вспомогательные функции парсинга)
...
**Цель:** Дополнить DataAggregator извлечением стоимостных полей
...
- [x] Вычисляет `resources_cost = total_cost - materials_cost` (если materials_cost доступен)
...
**Результат:** DataAggregator теперь корректно извлекает и вычисляет все стоимостные поля, совместимые со schema

---

### ✅ Задача 2.2.2: HOTFIX - Исправление database schema

**Статус:** ✅ ВЫПОЛНЕНО (2025-10-19)
**Зависимости:** Задача 1.1 ✅ (требует доработки)
**Параллельно с:** Задача 2.2.1 (исправление DataAggregator)

**Проблема 1:** Schema не имеет колонки для хранения composition (JSON)
- DataAggregator создает поле `composition` с JSON данными состава работ
- Schema: ❌ нет колонки для хранения

**Проблема 2:** resources.resource_type имеет несовместимый CHECK constraint
- Schema требует: `resource_type IN ('material', 'labor', 'machinery', 'equipment')`
- DataAggregator выдает: `row_type = 'Ресурс'` (не подходит!)
- Excel содержит: `Тип строки` со значениями: 'Расценка', 'Ресурс', 'Состав работ'

**Цель:** Исправить schema для совместимости с DataAggregator output

**Критерии успеха:**
- [x] Добавлена колонка `composition TEXT` в таблицу rates (для JSON хранения) ✅ schema.sql:42
- [x] Изменен CHECK constraint для resources.resource_type (выбран Вариант B) ✅ schema.sql:189
  - ✅ Удален CHECK constraint полностью
  - ✅ Колонка остается TEXT NOT NULL, принимает любые значения из Excel
- [x] Обновлены триггеры rates_fts_insert/update для включения composition в search_text ✅
  - ✅ rates_fts_insert: строки 113, 128
  - ✅ rates_fts_update: строки 144, 160
- [x] Обновлен комментарий к search_text для отражения изменений ✅ schema.sql:45

**Затронутые файлы:**
- ✅ `src/database/schema.sql` - все изменения применены
...
**Статус:** ✅ ЗАВЕРШЕНО
**Зависимости:** Задача 2.2.2 (решение по resource_type)
**Параллельно с:** Нет

**Проблема:**
Нужно определить логику маппинга `row_type` → `resource_type` для соответствия schema
...
**Затронутые файлы:**
- `src/utils/resource_classifier.py` (новый)
- `tests/test_resource_classifier.py` (новый)

---

### Задача 2.3: Популятор базы данных

**Зависимости:** 🚨 БЛОКИРОВАНО - Ждет завершения Задач 2.2.1, 2.2.2, 2.2.3
**Параллельно с:** Нет (зависит от исправлений)

**Цель:** Загрузить агрегированные данные в SQLite

**Критерии успеха:**
- [ ] Файл `src/etl/db_populator.py` содержит класс `DatabasePopulator`
- [ ] Метод `populate_rates()` использует executemany() для batch insert
- [ ] Размер batch оптимизирован (1000 записей за раз)
- [ ] Метод `populate_resources()` связывает ресурсы с расценками через rate_id
- [ ] Реализована транзакционность: либо все данные загружены, либо откат (rollback)
- [ ] Создание промежуточной таблицы для маппинга rate_code → rate_id
- [ ] Проверка уникальности rate_code перед вставкой
- [ ] Метод `clear_database()` для очистки перед повторной загрузкой
- [ ] Логирование прогресса с процентами выполнения
- [ ] Валидация данных после загрузки (SELECT COUNT(*) и сравнение)

**Затронутые файлы:**
- `src/etl/db_populator.py` (новый)

---

### Задача 2.4: Скрипт сборки базы данных

**Зависимости:** ⏳ После завершения Задач 2.1, 2.2, 2.3
**Параллельно с:** Нет (интегрирует все ETL компоненты)

**Цель:** Объединить все ETL шаги в единый pipeline

**Критерии успеха:**
- [ ] Файл `scripts/build_database.py` является исполняемым скриптом
- [ ] Принимает аргументы командной строки: --input (путь к Excel), --output (путь к БД), --force (пересоздать БД)
- [ ] Последовательно выполняет: load → aggregate → populate
- [ ] Выводит статистику: время выполнения, количество расценок, размер БД
- [ ] При ошибке на любом шаге - откатывает все изменения
- [ ] Создает резервную копию существующей БД перед перезаписью (если --force)
- [ ] Логирование в файл `data/logs/etl_YYYYMMDD_HHMMSS.log`
- [ ] Проверка целостности данных после сборки (PRAGMA integrity_check)

**Затронутые файлы:**
- `scripts/build_database.py` (новый)
- `data/logs/` (новая директория)
...
**Зависимости:** ⏳ После завершения Задачи 2.4 (нужна готовая БД для тестирования)
**Параллельно с:** Задачи 3.2, 3.3 (все три компонента search можно разрабатывать параллельно)

**Цель:** Реализовать полнотекстовый поиск с FTS5

**Критерии успеха:**
- [ ] Файл `src/search/search_engine.py` содержит класс `SearchEngine`
- [ ] Метод `search(query, filters, limit)` принимает естественноязыковый запрос
- [ ] Используется `fts_config.prepare_fts_query()` для подготовки FTS запроса
- [ ] Поддержка фильтров: unit_type (м2/м3/шт), min_cost, max_cost, category
- [ ] SQL запрос использует JOIN между rates и rates_fts
- [ ] Результаты ранжируются по FTS rank (чем ближе к 0, тем релевантнее)
- [ ] Возвращаемые поля: rate_code, rate_full_name, rate_short_name, unit_measure_full, cost_per_unit, total_cost, rank
- [ ] Метод `search_by_code(rate_code)` для точного поиска по коду (с поддержкой префиксов)
- [ ] Обработка случаев: нет результатов, слишком много результатов (>1000)
- [ ] Логирование всех поисковых запросов для аналитики

**Затронутые файлы:**
- `src/search/search_engine.py` (новый)
...
**Зависимости:** ⏳ После завершения Задачи 2.4 (нужна готовая БД для тестирования)
**Параллельно с:** Задачи 3.1, 3.3 (независимые компоненты)

**Цель:** Рассчитывать точную стоимость для заданного объема работ

**Критерии успеха:**
- [ ] Файл `src/search/cost_calculator.py` содержит класс `CostCalculator`
- [ ] Метод `calculate(rate_code, quantity)` выполняет расчет стоимости
- [ ] Формула: `calculated_cost = (total_cost / unit_quantity) * quantity`
- [ ] Аналогично рассчитываются: calculated_materials, calculated_resources
- [ ] Возвращает словарь с: rate_info, base_cost, cost_per_unit, calculated_total, materials, resources, quantity
- [ ] Валидация: rate_code существует, quantity > 0, unit_quantity не 0
- [ ] Метод `get_detailed_breakdown(rate_code, quantity)` возвращает разбивку по ресурсам
- [ ] Каждый ресурс пересчитывается пропорционально: `(resource.quantity / unit_quantity) * quantity`
- [ ] Округление до 2 знаков после запятой для всех денежных значений
- [ ] Обработка edge cases: отсутствие расценки, деление на 0

**Затронутые файлы:**
- `src/search/cost_calculator.py` (новый)
...
**Зависимости:** ⏳ После завершения Задачи 2.4 (нужна готовая БД для тестирования)
**Параллельно с:** Задачи 3.1, 3.2 (независимые компоненты)

**Цель:** Сравнивать несколько расценок между собой

**Критерии успеха:**
- [ ] Файл `src/search/rate_comparator.py` содержит класс `RateComparator`
- [ ] Метод `compare(rate_codes, quantity)` принимает список кодов и объем
- [ ] Для каждой расценки рассчитывается стоимость для заданного quantity
- [ ] Возвращает DataFrame (или список словарей) с колонками: rate_code, description, unit_type, cost_per_unit, total_for_quantity, materials_for_quantity
- [ ] Результаты отсортированы по total_for_quantity (от меньшего к большему)
- [ ] Добавлена колонка `difference_from_cheapest` (в рублях и процентах)
- [ ] Метод `find_alternatives(rate_code, max_results)` находит похожие расценки по search_text
- [ ] Валидация: все rate_codes существуют, quantity > 0
- [ ] Обработка случая: только один код в списке

**Затронутые файлы:**
- `src/search/rate_comparator.py` (новый)
...
**Зависимости:** ⏳ После завершения Задач 3.1, 3.2, 3.3 (нужно знать API всех модулей)
...
**Критерии успеха:**
- [ ] Файл `agents/estimates_agent_prompt.md` содержит детальную инструкцию для агента
- [ ] Определена роль: "Ты - эксперт-сметчик с доступом к базе строительных расценок"
- [ ] Перечислены доступные Python модули: SearchEngine, CostCalculator, RateComparator
- [ ] Описаны методы каждого класса с параметрами и примерами
- [ ] Определены правила работы: всегда использовать модули, не придумывать данные, проверять единицы измерения
- [ ] Примеры обработки типовых запросов с пошаговым алгоритмом:
  - "Сколько стоит X м² работы Y?"
  - "Сравни расценки A и B"
  - "Что дешевле: вариант 1 или вариант 2?"
  - "Детализируй расценку по ресурсам"
- [ ] Формат ответа: краткий ответ → детальный расчет → альтернативы → доп. информация
- [ ] Инструкции по обработке неоднозначности (задавать уточняющие вопросы)
- [ ] Примеры форматирования ответов (таблицы, списки, подсветка важного)
- [ ] Указание на необходимость пояснять откуда взялись цифры

**Затронутые файлы:**
- `agents/estimates_agent_prompt.md` (новый)

---

### Задача 4.2: Вспомогательные утилиты для агента

**Зависимости:** ⏳ После завершения Задач 3.1, 3.2, 3.3
**Параллельно с:** Задача 4.1 (независимые компоненты)

**Цель:** Создать функции-обертки для удобства использования в диалоге

**Критерии успеха:**
- [ ] Файл `src/utils/agent_helpers.py` содержит функции-обертки
- [ ] Функция `natural_search(query)` - обертка над SearchEngine.search() с красивым выводом
- [ ] Функция `quick_calculate(rate_code_or_description, quantity)` - поиск + расчет в одном вызове
- [ ] Функция `show_rate_details(rate_code)` - форматированный вывод расценки с ресурсами
- [ ] Функция `compare_variants(descriptions, quantity)` - поиск + сравнение по описаниям
- [ ] Все функции возвращают словари с результатами + formatted_text для вывода пользователю
- [ ] Используется rich/tabulate для красивого форматирования таблиц
- [ ] Обработка всех возможных ошибок с user-friendly сообщениями
- [ ] Кэширование результатов поиска в `data/cache/query_cache.json` (опционально)

**Затронутые файлы:**
- `src/utils/agent_helpers.py` (новый)
...
**Цель:** Подготовить набор тестовых запросов и ожидаемых результатов
...
**Зависимости:** ⏳ После завершения Задач 3.1, 7.1 (нужен SearchEngine и БД)
...
**Цель:** Простой веб-интерфейс для демонстрации
