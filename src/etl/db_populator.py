...
class DatabasePopulator:
    ...
    def clear_database(self) -> None:
        ...
        try:
            ...
            tables = ['resources', 'rates']

            for table in tables:
                delete_sql = f"DELETE FROM {table}"
                self.db_manager.execute_update(delete_sql)

                # Reset AUTOINCREMENT sequence for resources table
                if table == 'resources':
                    reset_sql = "DELETE FROM sqlite_sequence WHERE name = 'resources'"
        ...
        try:
            ...
            stats = {
                **self._statistics,
                ...
                'total_records': rates_count + resources_count
            }
